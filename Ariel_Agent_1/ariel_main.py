import discord, os, aiohttp, datetime, re, asyncio, logging
...
_dir_name = AGENT_DIR.name  # e.g. "Ariel_Agent_1"
import re as _re
...
class ArielAgentNode(discord.Client):
    ...
    async def on_ready(self):
        ...
        self.loop.create_task(self.bg_check_kanban())
    ...
    async def on_message(self, message):
        ...
        if getattr(self, "setup_mode", False):
            ...
            steps_config = [
                ...
                ("agent_name", "請輸入我的名字 (Agent Name)", "Agent 1"),
        ...
        status = await message.reply(f"📡 {self.name} 正在同步和思考...")
        
        # ✅ 重點修樹: create_task 避免阻塞 Discord Heartbeat (Can't keep up 問題)
        # on_message 必須立即返回，而非一路 await 到 Bridge 回假
            ...
            if resp.status == 202:
                ...
                while True:
                    ...
                    async with aiohttp.ClientSession() as s2:
                        async with s2.get(poll_url) as poll_resp:
